...
const checks: Array<[string, boolean]> = [
  ...
  ['orphaned catalog cleanup only runs when current catalog worker is not authorized', /async function cancelOrphanedCatalogMarkers\(\)[\s\S]*!productionRailwayBranch\(\) \|\| catalogWorkerEnabledForCurrentRevision\(\)[\s\S]*return;/.test(firstFiveRecovery)],
  ['orphaned catalog cleanup waits a deploy grace period and is age bounded', /CATALOG_TAKEOVER_GRACE_MS = 45_000[\s\S]*await sleep\(CATALOG_TAKEOVER_GRACE_MS\)[\s\S]*createdAt: \{ lt: cutoff \}/.test(firstFiveRecovery)],
