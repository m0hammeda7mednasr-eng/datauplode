...
const checks: Array<[string, boolean]> = [
  ...
  ['orphaned catalog cleanup is bounded, delayed, and age scoped', /CATALOG_TAKEOVER_GRACE_MS = 45_000[\s\S]*CATALOG_TAKEOVER_SWEEP_COUNT = 4[\s\S]*sweep <= CATALOG_TAKEOVER_SWEEP_COUNT[\s\S]*await sleep\(CATALOG_TAKEOVER_GRACE_MS\)[\s\S]*createdAt: \{ lt: cutoff \}/.test(firstFiveRecovery)],
