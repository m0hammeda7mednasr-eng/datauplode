...
const checks: Array<[string, boolean]> = [
  ...
  ["intentional canary gate tolerates only readiness 200 or 503", workflow.includes('readiness_http_code') && workflow.includes('!= "200"') && workflow.includes('!= "503"')],
  ["live database must be Supabase", workflow.includes("body?.database?.target !== 'supabase'")],
  ["exact Supabase project pin required", workflow.includes("body?.database?.projectRefPinned !== true") && workflow.includes("body?.database?.projectRefMatched !== true")],
  ["exact Railway revision checked before canary", workflow.includes("actualRevision !== expectedRevision")],
  ["production environment required", workflow.includes("body?.platform?.productionEnvironment !== true")],
  ["runtime broad-write gate remains disabled", workflow.includes("runtimeWriteGateEnabled !== false")],
  ["inventory autostart remains disabled", workflow.includes("inventoryAutostartConfigured !== false") && workflow.includes("inventoryAutostartEnabled !== false")],
  ["job recovery writes remain disabled", workflow.includes("jobRecoveryConfigured !== false") && workflow.includes("jobRecoveryShopifyWritesConfigured !== false") && workflow.includes("jobRecoveryEnabled !== false")],
  ["sheet import autostart remains disabled", workflow.includes("sheetImportAutostartConfigured !== false") && workflow.includes("sheetImportAutostartEnabled !== false")],
  ["catalog write gate explicitly required", workflow.includes("catalogWriteGateEnabled !== true")],
  ["catalog write token configuration required", workflow.includes("catalogWriteTokenConfigured !== true")],
  ["Google Sheet write gate remains disabled", workflow.includes("catalogSheetWriteGateEnabled !== false")],
  ["catalog dry-run safety remains configured", workflow.includes("catalogAuditDryRunConfigured !== true")],
  ["broad safe mode remains active", workflow.includes("configuration?.safeMode !== true")],
  ["canary max rows exactly one", workflow.includes("catalogCanaryMaxRows) !== 1")],
  ["baseline readiness 503 cannot claim baseline write safety", workflow.includes("readinessHttpCode === '503'") && workflow.includes("writeSafetyReady === true")],
  ...
  ["post-canary readiness tolerates only 200 or 503", workflow.split('readiness_http_code').length >= 5 && workflow.includes("after canary; rollout blocked")],
  ["post-canary Supabase project pin is rechecked", workflow.includes("Exact pinned Dabdoob Supabase binding failed after canary")],
  ["post-canary broad write paths are rechecked", workflow.includes("Broad or Sheet write safety changed during canary") && workflow.includes("Background write path became enabled during canary")],
