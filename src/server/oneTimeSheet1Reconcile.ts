...
export function startOneTimeSheet1Reconcile(port: number) {
  if (!isolatedFirstFiveWorkerEnabled()) {
    console.log(
      "[first5-reconcile] isolated worker blocked: requires Railway production branch stabilize-supabase-railway and SYNC_FIRST5_RECONCILE_ENABLED=true; SYNC_FIRST5_RECONCILE_DISABLED=true is the emergency kill switch",
    );
    return;
  }

  console.warn(
    "[first5-reconcile] isolated existing-products-only worker ENABLED; global runtime write gate remains independent",
  );
